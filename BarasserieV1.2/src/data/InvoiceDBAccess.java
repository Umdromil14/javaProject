...
public class InvoiceDBAccess {
    ...
    public InvoiceDBAccess() {
    }
    public List<Invoice> getInvoices (Integer id,Date start,Date end,String status) throws SQLException
    {
        List<Invoice> invoices = new ArrayList<>();
        //SELECT be.firstname, doc.inclVATTotal, doc.id as "invoice number", doc.payementDeadline,dt.code as type
        // from doc_status ds
        // join document doc on ds.code = doc.docStatus
        // join workflow wf on doc.workflow = wf.id
        // join business_entity be on wf.agent = be.id
        // join doc_type dt on doc.docType = dt.code
        // where ds.description = "invoice unpaid" and doc.payementDeadline >= now();
        StringBuilder query = new StringBuilder()
        .append("SELECT be.firstname, doc.inclVATTotal, doc.id as \"invoice_number\", doc.date,dt.code as type ")
        ...
        //need to get name client, price, number of the invoice, paymentdeadline
        return invoices;
    }
}
